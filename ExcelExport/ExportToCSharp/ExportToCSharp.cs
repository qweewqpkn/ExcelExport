...
class ExportToCSharp
{
    ...
    public static void Export()
    {
        ...
        mBinaryPath = sr.ReadLine();
        mCodePath = sr.ReadLine();

        //合并的配置
        FileStream fs = File.Open(mBinaryPath + "Config.byte", FileMode.Create, FileAccess.ReadWrite);
        ...
        for (int i = 0; i < allExcelPath.Length; i++)
        {
            ...
            if (extensionName == ".xlsx")
            {
                ...
                ExportExcelToCode(book.Tables[0], allExcelPath[i]);
                byte[] bytes = ExportExcelToByte(book.Tables[0], allExcelPath[i]);
    ...
    static byte[] ExportExcelToByte(DataTable sheet, string path)
    {
        ...
        {
            Console.WriteLine("file too long, check!!!!");
            return null;
        }
    }

    static void ExportExcelToCode(DataTable sheet, string path)
    {
